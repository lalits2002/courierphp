...
class CourierCompany extends BD_Controller {
...
//        $this->auth();
        ...
        $this->load->model('CourierCompanyModel');
    }
...
//		->callback('_other_func', 'Your error.')
    ...
    public function _checkDuplicate($email, $phone) {
        $email = $this->validation->get_data($email);
        $phone = $this->validation->get_data($phone);
        $result = $this->M_main->getCompany($email, $phone);
    ...
    public function company_post(){
        ...
        $this->validation->set_post($params);
         ...
         $this->validation->required(array('BUSINESSNAME', 'CONTACTPERSON', 'PHONENO', 'LOGINPWD'), 'Fields are required')
                ->email('EMAIL', 'Entered email is not valid')
                ->minlen('PHONENO', 10, 'Invalid length of phone')
                ->callback('_checkDuplicate', 'Phone or Email already exists.', array('EMAIL', 'PHONENO'));
    }
